...
//
// sub_76ED20 is a __stdcall bounded string copy:
//   int __stdcall sub_76ED20(char* dst, const char* src, int maxlen)
//   - If dst or src is NULL: SetLastError(0x57), return 0
//   - If maxlen == 0x7FFFFFFF: unbounded — copy until '\0', null-terminate, return length
//   - If maxlen != 0x7FFFFFFF: bounded — copy at most (maxlen-1) bytes, null-terminate, return length
//   - Empty source (*src == 0): null-terminate at dst[0], return 0
//
// IDA-verified assembly (0x76ED20, 2026-06-23). This rewrite matches the
// original byte-for-byte while using SSE2 for the copy body. Page-safe:
// no pre-scan, no unaligned loads crossing 4KB boundaries. SEH-guarded.
// ================================================================
