...
#define WOW_OPTIMIZE_VERSION_STR    "3.6.5"
...
#define TEST_DISABLE_CRT_CHAR_SSE2       1  // DISABLED: crash isolation - ERROR #132 on Circle/Warmane servers
...
// On Wine, MinHook patching WoW's .text section (0x00400000-0x00FFFFFF)
// invalidates rosettax87 JIT translations for those pages. If a WoW
// thread is executing translated code on that page at patch time →
// "rosetta error: no code fragment". System DLL hooks are safe because
// they're in separate modules with independent translation caches.
// Only available in TUs that include MinHook.h before version.h.
// ================================================================
// Guard: only compile if MH_STATUS is available (MinHook.h included first).
// Files using this must #include "MinHook.h" before #include "version.h".
