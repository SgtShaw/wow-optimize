...
// ================================================================
// WoW's D3D9 renderer calls SetRenderState/SetSamplerState/SetTexture
// thousands of times per frame, often setting the same value that's
// already current. We cache the last-set state and skip redundant
// calls, reducing D3D9 driver overhead significantly in raids with
// many units/spells on screen.
//
// SAFETY APPROACH (avoids the vtable-layout trap — lesson #12):
//   1. Hook d3d9.dll's Direct3DCreate9 export via MinHook (safe).
//   2. Intercept CreateDevice (vtable slot 16 on the returned
//      IDirect3D9*) to grab the ACTUAL function pointers at runtime.
//   3. MinHook-hook those function pointers directly (NOT vtable swap).
//      MinHook patches the function prologue, so it works identically
//      for native d3d9, DXVK Vulkan wrappers, and d3d9→OpenGL stacks.
//
// This auto-detects the device type at runtime. No compile-time
// assumptions about vtable layout.
//
// Hooks installed (when device is created):
//   - SetRenderState    (vtable idx 57): 256-slot compare-before-set
// ================================================================
