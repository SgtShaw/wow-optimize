    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Animation LOD (crowd FPS)", new SettingItem("Graphics_Sound", "AnimationLod", false, null, "Big FPS win in raids and packed cities. Throttles skeletal bone animation for distant/background models when the scene is crowded (they animate at ~30fps instead of 60 - imperceptible on far models). Nearby models are untouched, light scenes run at full rate. Only ever skips work, never changes what's on screen up close. Stand in Dalaran and watch your FPS.") },
                { "Nameplate Throttle (crowd FPS)", new SettingItem("Graphics_Sound", "NameplateThrottle", false, null, "FPS win in battlegrounds and packed cities with lots of nameplates. Throttles the per-frame cosmetic refresh (health-bar animation, threat color, alpha) for non-target nameplates when many are up - your current target stays full rate, and plate positioning is never touched so nothing lags behind its unit. Only ever skips work. Light scenes are untouched.") },
                ...
                { "Async Frustum Culling", new SettingItem("Graphics_Sound", "AsyncCulling", false, null, "Pre-computes visibility queries on two helper threads so the render path can reuse the result. Automatically stays off under Wine/Rosetta.") },
