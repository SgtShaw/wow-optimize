    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "D3D9Ex Vulkan DXVK Support", new SettingItem("General", "VulkanDXVK", true, null, "Optimizes DLL hook integration to work cleanly with DXVK (requires placing a d3d9.dll Vulkan wrapper in the game folder).") },
                ...
                { "SavedVariables Serializer", new SettingItem("Combat_Net", "SavedVarsSerializer", true, null, "Serializes and writes addon variables incrementally in a lock-free worker thread.") },
                { "SavedVariables Async Writer", new SettingItem("Combat_Net", "SavedVarsAsync", true, null, "Performs SavedVariables file flushing asynchronously in the background to prevent logout freezes.") },
                { "SavedVariables Preloader", new SettingItem("Combat_Net", "SavedVarsPretoken", true, null, "Pre-loads and parses addon configuration files during the early loading screen sequence.") },
