    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                // Combat & Net
                { "Packet Processing Throttle", new SettingItem("Combat_Net", "PacketProcessingThrottle", true, null, "Paces how many incoming packets are handled per frame. Runs on every install; this switch is new. Turn it OFF first if you are investigating a disconnect.") },
                { "Data Store Buffering", new SettingItem("Combat_Net", "CDataStoreBuffering", true, null, "Buffers reads out of the packet data store. Runs on every install; this switch is new. Turn it OFF if you are investigating a disconnect.") },
                { "Network String Dedup", new SettingItem("Combat_Net", "NetworkStringDedup", true, null, "Deduplicates repeated strings arriving from the server. Runs on every install; this switch is new. Turn it OFF if you are investigating a disconnect.") },
                { "GUID String Cache", new SettingItem("Combat_Net", "GuidStringCache", true, null, "Caches GUID-to-string conversions used by combat and unit code.") },
                { "Addon Message Limiter", new SettingItem("Combat_Net", "AddonMsgLimiter", true, null, "Rate-limits addon-to-addon chat traffic. Relevant to disconnects if a server punishes addon message volume.") },
                { "Combat Event Limit", new SettingItem("Combat_Net", "CombatEventLimit", true, null, "Caps how many combat events are processed in one frame.") },
                { "Combat Log Filter", new SettingItem("Combat_Net", "CombatLogFilter", true, null, "Drops combat log events nothing has subscribed to before they reach Lua.") },
                { "Combat Log Async", new SettingItem("Combat_Net", "CombatLogAsync", true, null, "Moves combat log formatting off the main thread.") },
                { "Aura Update Dedup", new SettingItem("Combat_Net", "AuraUpdateDedup", true, null, "Collapses duplicate aura updates that arrive in the same frame.") },
                { "Aura Preload Cache", new SettingItem("Combat_Net", "AuraPreloadCache", true, null, "Preloads aura icon and duration data so first application does not stall.") },
                { "Item Data Prefetch", new SettingItem("Combat_Net", "ItemDataPrefetch", true, null, "Fetches item data ahead of the frame that needs it.") },
                { "Spell Overlay Preload", new SettingItem("Combat_Net", "SpellOverlayPreload", true, null, "Preloads spell activation overlay textures.") },
                { "Nameplate Culling", new SettingItem("Combat_Net", "NameplateCulling", true, null, "Skips nameplate work for plates that are not on screen.") },
                { "Nameplate Distance CVar", new SettingItem("Combat_Net", "NameplateDistanceCvar", true, null, "Registers and manages the nameplate draw-distance CVar.") },
