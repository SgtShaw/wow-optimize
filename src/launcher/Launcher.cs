    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Segregated VA Arena (experimental)", new SettingItem("General", "VaArena", false, null, "EXPERIMENTAL anti-fragmentation: routes the client's large VirtualAlloc allocations into one pre-reserved 64MB block so they don't scatter holes through the 32-bit address space. Placement only - frees still go through Windows normally. Test connect + a full raid + exit before relying on it, and report if anything breaks.") },
