    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "SSE2 Matrix Multiply", new SettingItem("Graphics_Sound", "MatrixMultiplySse2", false, null, "Replaces the client's 4x4 matrix multiply, measured at 1.27% of main-thread execution. Compared against the original over 200000 matrices: both land the same distance from an exact double-precision result, so neither is more accurate. Never run in a game, so it is off by default.", true) },
                { "SSE2 Quaternion Normalize", new SettingItem("Graphics_Sound", "QuatNormalizeSse2", false, null, "Replaces the client's quaternion normalize, measured at 3.13% of main-thread execution in a CPU-bound profile. Checked against the original over 400000 quaternions: one float ULP of difference at worst. It has never been run in an actual game, which is why it is here and off by default.", true) },
                ...
                { "Vertex Buffer Preallocation", new SettingItem("General", "VertexBufferPrealloc", true, null, "Pools vertex buffer allocations for the D3D9 state cache instead of allocating per use. Runs on every install; this switch is new.") },
                { "M2 Matrix SSE2", new SettingItem("Graphics_Sound", "M2MatrixSimd", true, null, "SSE2 matrix copy for model transforms, plus the SIMD bone path. Runs on every install; this switch is new.") },
                { "Font Alpha Fast Path", new SettingItem("UI_Lua", "FontAlphaFastpath", true, null, "Skips redundant alpha state changes on the D3D9 text path. Runs on every install; this switch is new.") },
